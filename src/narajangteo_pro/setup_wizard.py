...
def run_wizard() -> None:
    # 헤드리스 환경(서버·CI·Docker)에서 import 자체가 실패하지 않도록 지연 import
    try:
        ...
        raise RuntimeError(
            f"GUI를 표시할 수 없습니다 ({e}). "
            "NARA_API_KEY 환경변수를 직접 설정하거나 "
            f"{_ENV_PATH} 파일에 NARA_API_KEY=<키> 를 작성하세요."
        ) from e
    ...
    tk.Label(frame, text="나라장터 MCP 서버 설정", font=("맑은 고딕", 14, "bold")).pack(anchor="w")
    tk.Label(
        frame,
        text="공공데이터포털(data.go.kr)에서 발급받은\n서비스키(Decoding 값)를 입력하세요.",
        justify="left",
        fg="#555",
    ...
    entry = tk.Entry(frame, textvariable=key_var, width=58)
    entry.pack(fill="x", pady=(4, 4))
    entry.focus()

    tk.Label(frame, text=f"저장 위치: {_ENV_PATH}", fg="#888", font=("맑은 고딕", 8)).pack(
        anchor="w", pady=(0, 16)
    )
...
def ensure_api_key() -> bool:
    ...
    # GUI 없는 환경이면 run_wizard가 RuntimeError를 던져 여기서 전파됨
    run_wizard()
